...
module.exports = (data, staticData) => {
  ...
  return {
    ...
    writeEnchantmentRegistry() {
      const rawEnchants = data.enchantmentsArray ?? staticData.enchantmentsArray

      return {
        id: "minecraft:enchantment",
        entries: rawEnchants.map(x => {
          return {
            key: `minecraft:${x.name}`,
            value: nbt.comp({
              slots: nbt.list([]), // TODO
              max_cost: nbt.comp({
                per_level_above_first: nbt.int(x.maxCost.a),
                base: nbt.int(x.maxCost.b+x.maxCost.a)
              }),
              min_cost: nbt.comp({
                per_level_above_first: nbt.int(x.minCost.a),
                base: nbt.int(x.minCost.b+x.minCost.a)
              }),
              supported_items: nbt.string(`#minecraft:enchantable/${x.category}`),
              max_level: nbt.int(x.maxLevel),
              weight: nbt.int(x.weight),
              effects: nbt.comp({}), // TODO
              anvil_cost: nbt.int(0), // TODO
              // primary_item: nbt.string(''), // TODO this field isn't always present
              description: nbt.comp({
                translate: nbt.string(`enchantment.minecraft.${x.name}`)
              })
            })
          }
        })
